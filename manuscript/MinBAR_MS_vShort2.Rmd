...
3. Three case studies are presented to assess the hypothesis and package functionality. 
...
To solve these problems, the idea behind *MinBAR* is to sequentially fit several concentric SDMs, each with an increased diameter, from the centre of the species geographical distribution to the periphery, until a model which satisfies user's needs is reached. We call "buffers" to these concentric SDMs.

## Evaluation metrics
One of the most widely used metrics to evaluate the performance of SDMs is the area under the receiver operating characteristic curve (i.e. AUC), although it has received several critiques [e.g. @RN6694; @RN6689]. In particular, for the purpose of *MinBAR*, AUC is not the best choice because it is highly influenced by the defined background area. For this reason, *MinBAR* uses the Boyce Index [@RN5422], implemented in the R package *ecospat* [@RN2970].
...
By default, the function *minba* defines 10 buffers, with 3 model replicates per buffer and lets the process produce models for all of them. MaxEnt was run using *maxnet* with default parameters, except for the number of background points. The intention of that was to limit interferences in the results as much as possible for all the species. We used 19 climatic variables available from WorldClim [@RN6688] at different resolutions depending on each case of study.

All the R scripts used in these case studies can be found at https://github.com/xavi-rp/MinBA. While *MinBAR* v.1.1.0 can be downloaded from CRAN at https://CRAN.R-project.org/package=MinBAR. The latest (unstable) version is available at https://github.com/xavi-rp/MinBAR.
...
`r fig_num("f4", display = "cite")` shows how the best models for most of the species were those fitted with only part of their distribution`r prop_bal`, confirming Case 1 results. On the other hand, the comparison of the rasters of probability of presence for *MinBAR*’s best buffers with the actual distribution of the virtual species (`r table_num("tb2", display = "cite")`) gave an average of `r paste0(round(comp_BestModl_VirtSp_MeanSD[1, 3], 2), " (SD = ", round(comp_BestModl_VirtSp_MeanSD[2, 3], 2), ")")` and `r paste0(round(comp_BestModl_VirtSp_MeanSD[1, 4], 2), " (SD = ", round(comp_BestModl_VirtSp_MeanSD[2, 4], 2), ")")` for D and I respectively.
...
Both BI_tot and BI_part for most of the species did not improve very much when increasing the buffers after the first half. `r table_num("tb3", display = "cite")` and `r fig_num("f5", display = "cite")` show that the best models for most of the species were those fit with only part of their distribution `r prop_bal`. The output graphs produced by *minba* can be seen in Supplementary Material S`r ts + 1`. 
